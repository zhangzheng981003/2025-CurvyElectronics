...
{
public:
	CutMesh(Mesh mesh, std::vector<int> init_seam = {})
	{
		ori_mesh_ = mesh;
	...
	int GetPatchNum()
	{
		return cut_patches_.size();
	}

	bool GetPatch(int i, Mesh& mesh)
	{
		if (i < 0 || i > cut_patches_.size()) return false;
	...
	void Compute(std::string fileDirPath);
	...
	void Compute()
	{
		...
		OpenMesh::IO::write_mesh(cut_mesh, "bound_mesh.obj", 0x0000, 12);
		...
		for (int i = 0; i < component_num; i++)
		{
			...
			const std::string seg_mesh_name = "Patches/Patches/seg_" +
				std::to_string(i) + ".obj";
			MeshTools::WriteMesh(temp_mesh, seg_mesh_name);

			cut_patches_.push_back(temp_mesh);
		}
	...
	{
		std::vector<bool> seam_status(ori_mesh_.n_edges(), false);
