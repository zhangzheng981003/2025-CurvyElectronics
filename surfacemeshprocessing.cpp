...
void SurfaceMeshProcessing::createActions()
{
	...
	loadSeg = new QAction("Load Seg", this);
	connect(loadSeg, SIGNAL(triggered()), viewer, SLOT(LoadSeg()));
}
...
void SurfaceMeshProcessing::createToolBars()
{
	...
	localOperationBar->addSeparator();
	localOperationBar->addAction(loadMesh);
	localOperationBar->addAction(loadSeg);
}
